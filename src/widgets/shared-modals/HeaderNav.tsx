...
function HeaderNavInner() {
    ...
    const isFunding = pathname === "/funding";
    ...
    return (
        <header suppressHydrationWarning className="fixed top-0 left-0 right-0 z-50 h-12 w-full bg-[var(--surface-card)] border-b border-[var(--border-subtle)] flex items-center px-4">
            ...
            <nav className="flex items-center gap-1" aria-label="주요 메뉴">
                ...
                <Link href="/funding" className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs transition-colors ${isFunding ? activeClass : inactiveClass}`} aria-current={isFunding ? "page" : undefined}>
                    <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="hidden sm:inline whitespace-nowrap">펀딩비</span>
                </Link>

...
export function SimOrderPanel({ account, totalUnrealizedPnl, totalPositionMargin, loading, onSubmit, onReset, clickedPrice, lockedMarginMode, positionCount = 0, orderCount = 0, isEn = false }: Props) {
    ...
    // 초기화 모달: Esc로 닫기 (처리 중에는 잠금)
    useEffect(() => {
        if (!resetOpen) return;
        const onKey = (e: KeyboardEvent) => {
            if (e.key === "Escape" && !resetting) setResetOpen(false);
        };
                            ...
                            >
                                ...
                                <div className={`rounded-xl px-4 py-3 space-y-2 ${isLight ? "bg-neutral-50 border border-neutral-200" : "bg-neutral-800/60 border border-zinc-700/50"}`}>
                                    ...
                                    <div className="flex justify-between items-center text-[12px]">
                                        ...
                                        <span className="font-mono font-bold tabular-nums flex items-center gap-1.5">
                                            ...
                                            <span className={isLight ? "text-neutral-900" : "text-white"}>$10,000.00</span>
                                        </span>
                                    </div>
                                </div>
                                ...
                                {resetError && (
                                    <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl bg-red-500/8 border border-red-500/15">
                                        ...
                                        <span className="text-[10px] text-red-500 leading-relaxed">{resetError}</span>
                                    </div>
                                )}
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        disabled={resetting}
                                        className="flex-1 py-2.5 text-[12px] font-bold rounded-xl border bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500 hover:text-white hover:border-red-500 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-red-500/10 disabled:hover:text-red-400"
                                    >
                                        {resetting ? (
                                            <span className="flex items-center justify-center gap-2">

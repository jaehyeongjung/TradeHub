...
async function fetchUsdKrw(): Promise<{ value: number; status: number }> {
    ...
    if (r1.ok) {
        ...
        if (isNum(j?.rates?.KRW))
            return { value: j.rates!.KRW!, status: r1.status };
    }
    // 2차
    ...
    if (r2.ok) {
        ...
        if (isNum(j2?.rates?.KRW))
            return { value: j2.rates!.KRW!, status: r2.status };
    }
...
async function fetchBinancePrice(
    ...
    const path = (host: string) =>
        `https://${host}/api/v3/ticker/price?symbol=${encodeURIComponent(
            symbol
        )}`;
    const headers = {
        "User-Agent": "TradeHub/1.0 (+https://www.tradehub.kr)",
    ...
    for (const host of hosts) {
        try {
            const r = await withTimeout(path(host), 3500, {
                headers,
                cache: "no-store",
            });
            lastStatus = r.status;

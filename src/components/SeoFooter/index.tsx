...
export default function SeoFooter() {
    ...
    const cardHover = isLight
        ...
        : "hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20 hover:border-neutral-700";
    ...
    const GuideLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
        <Link
            href={href}
            className={`group flex items-center justify-between py-1.5 text-[11px] transition-colors ${linkColor}`}
        >
            <span>{children}</span>
            <svg
                ...
                <div className="flex items-start gap-4 mb-8">
                    ...
                    <div className="flex-1 min-w-0">
                        <h2
                            ...
                            className={`text-sm font-bold leading-tight mb-1 ${titleColor}`}
                        >
                            TradeHub
                        </h2>
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
                    ...
                    <article className={`rounded-2xl p-5 transition-all duration-200 ${cardBg} ${cardHover}`}>
                        ...
                        <h3 className={`text-xs font-bold mb-2 leading-snug ${subTitleColor}`}>
                            <Link
                                href="/trading"
                                className={`transition-colors ${isLight ? "hover:text-teal-600" : "hover:text-teal-400"}`}
                            >
                                비트코인 모의투자
                            </Link>
                        </h3>
                    ...
                    <article className={`rounded-2xl p-5 transition-all duration-200 ${cardBg} ${cardHover}`}>
                        ...
                        <ul className={`divide-y ${isLight ? "divide-neutral-100" : "divide-neutral-800"}`}>
                            <li><GuideLink href="/guide/kimchi-premium">김치프리미엄이란?</GuideLink></li>
                            <li><GuideLink href="/guide/fear-greed-index">공포탐욕지수 보는법</GuideLink></li>
                            <li><GuideLink href="/guide/bitcoin-paper-trading">비트코인 모의투자 하는법</GuideLink></li>
                        </ul>
                        <div className="pt-2">
                            <Link
                                href="/guide"
                                className={`group flex items-center gap-1 text-[10px] font-medium transition-colors ${moreColor}`}
                            >
                                전체 가이드
                                <svg
                                    className="w-3 h-3 transition-transform duration-200 group-hover:translate-x-0.5"

...
export default function KimchiWidget({
  ...
  const color =
    pct == null ? "text-zinc-400" : pct >= 0 ? "text-emerald-500" : "text-rose-500";

  const fxAdjTxt =
    data?.upstream?.fxAdj && data?.upstream?.usdtUsd
      ? ` (FX×${(data.upstream.fxAdj).toFixed(3)}, USDT/USD×${data.upstream.usdtUsd.toFixed(3)})`
      : "";

  return (
    <div className="rounded-2xl border border-zinc-800 bg-neutral-950 p-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-white">김치 프리미엄</h3>
      ...
      <dl className="mt-2 grid grid-cols-2 gap-2 text-xs text-zinc-400">
        ...
        <dd className="text-right text-emerald-300 whitespace-nowrap">
          ...
          {data ? (
            <span className="ml-1 text-[10px] text-zinc-500">
              ({data.upbitPriceSource === "orderbook_mid" ? "호가 MID" : "체결가"})
            </span>
          ) : null}
        </dd>
        ...
        <dt>USD/KRW{fxAdjTxt}</dt>
        <dd className="text-right text-zinc-200">
          {data ? `${Math.round(data.usdkrw).toLocaleString()}원` : "—"}
        </dd>
      </dl>

      {/* 오류 표시를 원하면 주석 해제 */}
      {/* {error && <p className="mt-2 text-xs text-rose-400">업데이트 실패: {error}</p>} */}
    </div>
  );
}

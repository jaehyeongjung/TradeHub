...
export default function CoinChart({
      ...
      const { data: sub } = supabase.auth.onAuthStateChange((_e, s) => {
        ...
        if (nuid) {
          supabase
            ...
            .maybeSingle()
            .then(({ data: r }) => {
              if (r?.symbol) setSym(String(r.symbol).toUpperCase());
            });
        } else {
  ...
  // --- tickSize -> decimals/minMove 변환 helpers ---
  ...
  const minMoveFromTickSize = (tick: string) => {
    const n = Number(tick);
    return Number.isFinite(n) ? n : 0.01; // fallback
  };
  ...
  useEffect(() => {
    ...
    const candleSeries = chart.addSeries(CandlestickSeries, {
      ...
      borderVisible: false,
      // priceFormat은 아래에서 심볼의 tickSize 로 동적으로 덮어씀
    });

    // --- 심볼의 tickSize 로 가격 포맷 설정 ---
    (async () => {
      try {
      ...
      >
        ...
        <AnimatePresence>
          {hovered && (
            ...
            >
              <div className="font-semibold text-amber-300 mb-1">심볼 변경 안내</div>
              <p className="leading-snug">
                차트를 <b>클릭</b>하면 코인 심볼을 변경할 수 있습니다.
                <br />

...
export default function Chat({ roomId = "lobby" }: { roomId?: string }) {
  ...
  return (
    ...
    <div className="h-full flex flex-col p-3"> {/* ← 고정높이 삭제, h-full로 */}
      {/* 스크롤 영역 */}
      <div
        ref={listRef}
      ...
      >
        {msgs.map((m) => (
          <div key={m.id} className="text-sm border rounded px-3 py-2">
            <b className={m.user_id === userId ? "text-emerald-700" : "text-gray-700"}>
        ...
        {!msgs.length && (
          <div className="text-xs text-gray-600">아직 메시지가 없습니다.</div>
        )}
      </div>

      {/* 입력바 — 스크롤 영역 '밖'이라 항상 하단에 붙음 */}
      <div className="border-t pt-2 bg-white"> {/* sticky 제거, 여백 원하면 pt-2만 조절 */}
